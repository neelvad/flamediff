...
[![ci](https://github.com/neelvad/flamediff/actions/workflows/ci.yml/badge.svg)](https://github.com/neelvad/flamediff/actions/workflows/ci.yml)
[![PyPI](https://img.shields.io/pypi/v/flamediff)](https://pypi.org/project/flamediff/)
...
attribution over a run's checkpoints. Correlated signals are grouped into **incidents** (one
underlying cause fires many series at once), and every signal carries a *why* (a churn breakdown,
or the drift attribution):
...
```bash
pip install flamediff          # or: uv tool install flamediff
flamediff --help
```

(For development, clone and `uv sync` — see [Development](#development).) The reference
**fixtures are large and not checked in** (see [Reference fixtures](#reference-fixtures)
...
Docs: [`plan.md`](plan.md) for the design, [`ROADMAP.md`](ROADMAP.md) for the product plan,
